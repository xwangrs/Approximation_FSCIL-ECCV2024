...
class Tokenizer(nn.Module):
    def __init__(self,
        ...
        # exit()
        # self.masks = self.generate_mask(32)
    ...
    def generate_mask(self, n):
        ...
        # 创建一个全零张量
        masks = torch.ones(64, n , n) * 1e-7

        # 遍历所有的t=3的可能结果
        for i in range(0, n, 4):
            # print(i)
            for j in range(0, n, 4):
                masks[int(j/4+2*i), i:i+4, j:j+4] = torch.ones(4, 4)
                # print(int(j/4+2*i))
                # print(i)
                # print(j)
        # exit()
        # masks.requires_grad = True
        return nn.Parameter(masks)


    def forward(self, x):
        # x = self.conv_layers[0][0](x)
        # # self.masks = self.generate_mask(32)

        # # print(self.masks)
        # x = x.cuda() * self.masks.cuda()
