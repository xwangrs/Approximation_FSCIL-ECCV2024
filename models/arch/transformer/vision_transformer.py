...
from models.arch.transformer.utils import trunc_normal_
...
class VisionTransformer(nn.Module):
    ...
    def interpolate_pos_encoding(self, x, w, h):
        ...
        # print(patch_pos_embed.reshape(1, int(math.sqrt(N)), int(math.sqrt(N)), dim).permute(0, 3, 1, 2).size())
...
def vit_cifar32(patch_size=1, **kwargs):
    model = VisionTransformer(
        patch_size=patch_size, embed_dim=24, depth=3, num_heads=2, mlp_ratio=1,
        qkv_bias=True, **kwargs)
    return model

# def vit_small(patch_size=16, **kwargs):
#     model = VisionTransformer(
#         patch_size=patch_size, embed_dim=256, depth=7, num_heads=4, mlp_ratio=2,
#         qkv_bias=True, **kwargs)
#     return model

def vit_small(patch_size=2, **kwargs):
    model = VisionTransformer(img_size=[224],
        patch_size=patch_size, embed_dim=384, depth=12, num_heads=4, mlp_ratio=2,
